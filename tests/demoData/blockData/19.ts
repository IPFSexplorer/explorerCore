export default {
    page: 1,
    totalPages: 1,
    itemsOnPage: 1000,
    hash: "00000000841cb802ca97cf20fb9470480cae9e5daa5d06b4a18ae2d5dd7f186f",
    previousBlockHash:
        "000000008693e98cf893e4c85a446b410bb4dfa129bd1be582c09ed3f0261116",
    nextBlockHash:
        "0000000067a97a2a37b8f190a17f0221e9c3f4fa824ddffdc2e205eae834c8d7",
    height: 19,
    confirmations: 611896,
    size: 215,
    time: 1231564974,
    version: 1,
    merkleRoot:
        "9b9e461221e5284f3bfe5656efdc8c7cc633b2f1beef54a86316bf2ae3a3e230",
    nonce: "766824726",
    bits: "1d00ffff",
    difficulty: "1",
    txCount: 1,
    txs: [
        {
            txid:
                "9b9e461221e5284f3bfe5656efdc8c7cc633b2f1beef54a86316bf2ae3a3e230",
            vin: [{ n: 0, isAddress: false, value: "0" }],
            vout: [
                {
                    value: "5000000000",
                    n: 0,
                    addresses: ["1P9VmZogiic8d5ZUVZofrdtzXgtpbG9fop"],
                    isAddress: true
                }
            ],
            blockHash:
                "00000000841cb802ca97cf20fb9470480cae9e5daa5d06b4a18ae2d5dd7f186f",
            blockHeight: 19,
            confirmations: 611896,
            blockTime: 1231564974,
            value: "5000000000",
            valueIn: "0",
            fees: "0"
        }
    ]
};
