export default {
    page: 1,
    totalPages: 1,
    itemsOnPage: 1000,
    hash: "000000001884b26b0a2482be00f3dc03131154430462453c7193b6079daaf0eb",
    previousBlockHash:
        "00000000dda07b33ea6dc860805e868c05f8ffa2e8d35a8157a51ec64f0818f0",
    nextBlockHash:
        "00000000a639917e11de9c36aa5d2e5f3352c2fcef06729ac9540b84837c59a1",
    height: 129,
    confirmations: 611786,
    size: 215,
    time: 1231683160,
    version: 1,
    merkleRoot:
        "9a1f38fa9f25bb77504cfda998c4742b5a22466acb32ebedb247bd284682a5be",
    nonce: "1805656598",
    bits: "1d00ffff",
    difficulty: "1",
    txCount: 1,
    txs: [
        {
            txid:
                "9a1f38fa9f25bb77504cfda998c4742b5a22466acb32ebedb247bd284682a5be",
            vin: [{ n: 0, isAddress: false, value: "0" }],
            vout: [
                {
                    value: "5000000000",
                    n: 0,
                    addresses: ["1BF5nbuKF3UTYCQTpxuH62UA8WJWAqQKo5"],
                    isAddress: true
                }
            ],
            blockHash:
                "000000001884b26b0a2482be00f3dc03131154430462453c7193b6079daaf0eb",
            blockHeight: 129,
            confirmations: 611786,
            blockTime: 1231683160,
            value: "5000000000",
            valueIn: "0",
            fees: "0"
        }
    ]
};
