export default {
    page: 1,
    totalPages: 1,
    itemsOnPage: 1000,
    hash: "000000003ff1d0d70147acfbef5d6a87460ff5bcfce807c2d5b6f0a66bfdf809",
    previousBlockHash:
        "00000000174a25bb399b009cc8deff1c4b3ea84df7e93affaaf60dc3416cc4f5",
    nextBlockHash:
        "000000008693e98cf893e4c85a446b410bb4dfa129bd1be582c09ed3f0261116",
    height: 17,
    confirmations: 611898,
    size: 215,
    time: 1231563791,
    version: 1,
    merkleRoot:
        "a3e0b7558e67f5cadd4a3166912cbf6f930044124358ef3a9afd885ac391625d",
    nonce: "2474926087",
    bits: "1d00ffff",
    difficulty: "1",
    txCount: 1,
    txs: [
        {
            txid:
                "a3e0b7558e67f5cadd4a3166912cbf6f930044124358ef3a9afd885ac391625d",
            vin: [{ n: 0, isAddress: false, value: "0" }],
            vout: [
                {
                    value: "5000000000",
                    n: 0,
                    addresses: ["1LPBetDzQ3cYwqQepg4teFwR7FnR1TkMCM"],
                    isAddress: true
                }
            ],
            blockHash:
                "000000003ff1d0d70147acfbef5d6a87460ff5bcfce807c2d5b6f0a66bfdf809",
            blockHeight: 17,
            confirmations: 611898,
            blockTime: 1231563791,
            value: "5000000000",
            valueIn: "0",
            fees: "0"
        }
    ]
};
