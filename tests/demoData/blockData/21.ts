export default {
    page: 1,
    totalPages: 1,
    itemsOnPage: 1000,
    hash: "000000006f016342d1275be946166cff975c8b27542de70a7113ac6d1ef3294f",
    previousBlockHash:
        "0000000067a97a2a37b8f190a17f0221e9c3f4fa824ddffdc2e205eae834c8d7",
    nextBlockHash:
        "0000000098b58d427a10c860335a21c1a9a7639e96c3d6f1a03d8c8c885b5e3b",
    height: 21,
    confirmations: 611894,
    size: 215,
    time: 1231566553,
    version: 1,
    merkleRoot:
        "e0175970efb4417950921bfcba2a3a1e88c007c21232ff706009cc70b89210b4",
    nonce: "3592540203",
    bits: "1d00ffff",
    difficulty: "1",
    txCount: 1,
    txs: [
        {
            txid:
                "e0175970efb4417950921bfcba2a3a1e88c007c21232ff706009cc70b89210b4",
            vin: [{ n: 0, isAddress: false, value: "0" }],
            vout: [
                {
                    value: "5000000000",
                    n: 0,
                    addresses: ["1Fi7o3BKMcT82NVtnMRNqsj8aE5CWbAo4z"],
                    isAddress: true
                }
            ],
            blockHash:
                "000000006f016342d1275be946166cff975c8b27542de70a7113ac6d1ef3294f",
            blockHeight: 21,
            confirmations: 611894,
            blockTime: 1231566553,
            value: "5000000000",
            valueIn: "0",
            fees: "0"
        }
    ]
};
