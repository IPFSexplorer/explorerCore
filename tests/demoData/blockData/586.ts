export default {
    page: 1,
    totalPages: 1,
    itemsOnPage: 1000,
    hash: "000000000d0d23516c5efd3af4eb951603bb30b2c93884b522a318b30e918ee7",
    previousBlockHash:
        "0000000046ff6c4f9a7224aa331c407c7e37f49434571307d6fca58695bcba38",
    nextBlockHash:
        "00000000fa041c4905c4eca1cc0c75204c2cd118da3ffd129c41840152038d20",
    height: 586,
    confirmations: 611330,
    size: 1071,
    time: 1232029520,
    version: 1,
    merkleRoot:
        "197b3d968ce463aa5da7d8eeba8af35eba80ded4e4fe6808e6cc0dd1c069594d",
    nonce: "932288301",
    bits: "1d00ffff",
    difficulty: "1",
    txCount: 3,
    txs: [
        {
            txid:
                "d45724bacd1480b0c94d363ebf59f844fb54e60cdfda0cd38ef67154e9d0bc43",
            vin: [{ n: 0, isAddress: false, value: "0" }],
            vout: [
                {
                    value: "5000000000",
                    n: 0,
                    addresses: ["1LZaMRQVdtHnrSGBVqdeaB1QfAdqFkXxoD"],
                    isAddress: true
                }
            ],
            blockHash:
                "000000000d0d23516c5efd3af4eb951603bb30b2c93884b522a318b30e918ee7",
            blockHeight: 586,
            confirmations: 611330,
            blockTime: 1232029520,
            value: "5000000000",
            valueIn: "0",
            fees: "0"
        },
        {
            txid:
                "4d6edbeb62735d45ff1565385a8b0045f066055c9425e21540ea7a8060f08bf2",
            vin: [
                {
                    n: 0,
                    addresses: ["1DNdPgBZRWjDj1JbVZQEYMv7jvqJF7R4Py"],
                    isAddress: true,
                    value: "5000000000"
                },
                {
                    n: 1,
                    addresses: ["1ELmSkQWnqgbBZNzxAZHts3MEYCngqRBeD"],
                    isAddress: true,
                    value: "5000000000"
                },
                {
                    n: 2,
                    addresses: ["1ADpf5rHERc2PmVAZZFoH7WbougKvkPDVD"],
                    isAddress: true,
                    value: "5000000000"
                },
                {
                    n: 3,
                    addresses: ["1ACWHyRM8rtbt96KauPJprnF2qDQSdPJ54"],
                    isAddress: true,
                    value: "5000000000"
                },
                {
                    n: 4,
                    addresses: ["1LfjLrBDYyPbvGMiD9jURxyAupdYujsBdK"],
                    isAddress: true,
                    value: "5000000000"
                }
            ],
            vout: [
                {
                    value: "25000000000",
                    n: 0,
                    addresses: ["19QKDUJtx9n7Vaga6nX1bVHdsnT4Khfyi6"],
                    isAddress: true
                }
            ],
            blockHash:
                "000000000d0d23516c5efd3af4eb951603bb30b2c93884b522a318b30e918ee7",
            blockHeight: 586,
            confirmations: 611330,
            blockTime: 1232029520,
            value: "25000000000",
            valueIn: "25000000000",
            fees: "0"
        },
        {
            txid:
                "6bf363548b08aa8761e278be802a2d84b8e40daefe8150f9af7dd7b65a0de49f",
            vin: [
                {
                    n: 0,
                    addresses: ["1E5npMTmh9PnBHxCacuqzAZRGEybkxPMAq"],
                    isAddress: true,
                    value: "5000000000"
                }
            ],
            vout: [
                {
                    value: "5000000000",
                    n: 0,
                    addresses: ["1QKJnFPvRoZ7avBhxqwxhsEv62JWYoB1R9"],
                    isAddress: true
                }
            ],
            blockHash:
                "000000000d0d23516c5efd3af4eb951603bb30b2c93884b522a318b30e918ee7",
            blockHeight: 586,
            confirmations: 611330,
            blockTime: 1232029520,
            value: "5000000000",
            valueIn: "5000000000",
            fees: "0"
        }
    ]
};
