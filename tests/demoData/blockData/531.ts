export default {
    page: 1,
    totalPages: 1,
    itemsOnPage: 1000,
    hash: "00000000a8eb67a3850b2a212994caf169bd08e56841d1777ed40e3057818fff",
    previousBlockHash:
        "0000000012ac8aedef5fb255a9a01f52d2292c5af21c737eaba1ef05e2c9c136",
    nextBlockHash:
        "0000000054dfed74457e592658485bb93f60e7e608c815f093e1d59106ab74e4",
    height: 531,
    confirmations: 611385,
    size: 216,
    time: 1231989040,
    version: 1,
    merkleRoot:
        "6a88880a33d8b5915704f921583189270381f85eae48087f7790aca5f291bfb2",
    nonce: "4249996831",
    bits: "1d00ffff",
    difficulty: "1",
    txCount: 1,
    txs: [
        {
            txid:
                "6a88880a33d8b5915704f921583189270381f85eae48087f7790aca5f291bfb2",
            vin: [{ n: 0, isAddress: false, value: "0" }],
            vout: [
                {
                    value: "5000000000",
                    n: 0,
                    addresses: ["1LPTq6DKLLTcqm2mYY5s5HdcWrJmuNZvhd"],
                    isAddress: true
                }
            ],
            blockHash:
                "00000000a8eb67a3850b2a212994caf169bd08e56841d1777ed40e3057818fff",
            blockHeight: 531,
            confirmations: 611385,
            blockTime: 1231989040,
            value: "5000000000",
            valueIn: "0",
            fees: "0"
        }
    ]
};
