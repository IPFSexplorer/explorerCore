export default {
    page: 1,
    totalPages: 1,
    itemsOnPage: 1000,
    hash: "0000000066356691a4353dd8bdc2c60da20d68ad34fff93d8839a133b2a6d42a",
    previousBlockHash:
        "00000000b66ac539d2cdfebdd4720cb08603b48edec56cc07273c40b082b1d58",
    nextBlockHash:
        "000000004da68466ee873c7095c766baf62df93a16df579350e01e7f78911616",
    height: 221,
    confirmations: 611694,
    size: 417,
    time: 1231770060,
    version: 1,
    merkleRoot:
        "67f445c708d28c16aeb6eaa81d7ccff29a0c39ff48c6d6298c9ff1e0b6899f07",
    nonce: "1772122369",
    bits: "1d00ffff",
    difficulty: "1",
    txCount: 2,
    txs: [
        {
            txid:
                "264e7238ddc574d5d6ded85d77bfcb69d99a0de17dfcdef0b7e97d430c4f202d",
            vin: [{ n: 0, isAddress: false, value: "0" }],
            vout: [
                {
                    value: "5000000000",
                    n: 0,
                    addresses: ["1BW3idasrBsovBWDzauf1yTh6QWK9UUEj5"],
                    isAddress: true
                }
            ],
            blockHash:
                "0000000066356691a4353dd8bdc2c60da20d68ad34fff93d8839a133b2a6d42a",
            blockHeight: 221,
            confirmations: 611694,
            blockTime: 1231770060,
            value: "5000000000",
            valueIn: "0",
            fees: "0"
        },
        {
            txid:
                "298ca2045d174f8a158961806ffc4ef96fad02d71a6b84d9fa0491813a776160",
            vin: [
                {
                    n: 0,
                    addresses: ["1LzBzVqEeuQyjD2mRWHes3dgWrT9titxvq"],
                    isAddress: true,
                    value: "100000000"
                }
            ],
            vout: [
                {
                    value: "100000000",
                    n: 0,
                    addresses: ["1BDvQZjaAJH4ecZ8aL3fYgTi7rnn3o2thE"],
                    isAddress: true
                }
            ],
            blockHash:
                "0000000066356691a4353dd8bdc2c60da20d68ad34fff93d8839a133b2a6d42a",
            blockHeight: 221,
            confirmations: 611694,
            blockTime: 1231770060,
            value: "100000000",
            valueIn: "100000000",
            fees: "0"
        }
    ]
};
