export default {
    page: 1,
    totalPages: 1,
    itemsOnPage: 1000,
    hash: "00000000fd4afcc15f0fdda9b24be4c62068d8cf82fe6277730fd096712d9d08",
    previousBlockHash:
        "00000000a7d8beee715bbcd62dc818573a8ae9fe398c7d39487e9e776e2852c0",
    nextBlockHash:
        "0000000016a9f3ddedb8e850941f6981623c7721302a32ca8548dff1f7bd6e58",
    height: 175,
    confirmations: 611740,
    size: 215,
    time: 1231733876,
    version: 1,
    merkleRoot:
        "48fb3d15e828a31aab6ae17b6f5b14333d0701a9358f9d2ea57655c32cb53b90",
    nonce: "2262400290",
    bits: "1d00ffff",
    difficulty: "1",
    txCount: 1,
    txs: [
        {
            txid:
                "48fb3d15e828a31aab6ae17b6f5b14333d0701a9358f9d2ea57655c32cb53b90",
            vin: [{ n: 0, isAddress: false, value: "0" }],
            vout: [
                {
                    value: "5000000000",
                    n: 0,
                    addresses: ["1917k1LUodPbbjJenczkVaGyJpSj8CqYTC"],
                    isAddress: true
                }
            ],
            blockHash:
                "00000000fd4afcc15f0fdda9b24be4c62068d8cf82fe6277730fd096712d9d08",
            blockHeight: 175,
            confirmations: 611740,
            blockTime: 1231733876,
            value: "5000000000",
            valueIn: "0",
            fees: "0"
        }
    ]
};
