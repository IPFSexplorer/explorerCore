export default {
    page: 1,
    totalPages: 1,
    itemsOnPage: 1000,
    hash: "000000001fa8db1f15a9abd900cce0df7823a8397f2728b1d275e8fe0c0d2df0",
    previousBlockHash:
        "0000000069babcb34cc43c80e1c43d423a70821c740193a8265d3edc204ce4f0",
    nextBlockHash:
        "0000000021a406d58deeb398942e3c6cf956295fc11b192935c453c7d80be1ca",
    height: 94,
    confirmations: 611821,
    size: 215,
    time: 1231655581,
    version: 1,
    merkleRoot:
        "3226a75aaf23565ab1f4b4d3cf7f945e188b1ef35d14bba50bd3ee05df985490",
    nonce: "1923907113",
    bits: "1d00ffff",
    difficulty: "1",
    txCount: 1,
    txs: [
        {
            txid:
                "3226a75aaf23565ab1f4b4d3cf7f945e188b1ef35d14bba50bd3ee05df985490",
            vin: [{ n: 0, isAddress: false, value: "0" }],
            vout: [
                {
                    value: "5000000000",
                    n: 0,
                    addresses: ["1PHG1AoZJ5yyg6MV2fkqYCuDDik2NTfzfm"],
                    isAddress: true
                }
            ],
            blockHash:
                "000000001fa8db1f15a9abd900cce0df7823a8397f2728b1d275e8fe0c0d2df0",
            blockHeight: 94,
            confirmations: 611821,
            blockTime: 1231655581,
            value: "5000000000",
            valueIn: "0",
            fees: "0"
        }
    ]
};
