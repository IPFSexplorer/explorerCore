export default {
    page: 1,
    totalPages: 1,
    itemsOnPage: 1000,
    hash: "0000000074e77a1e561ab5247bec8a977977d3dde3d70a74fabf0adc3d6bf4ac",
    previousBlockHash:
        "00000000c3a81d9932edc9a2e3358e631b094d9628178bbf6089b12f75b08815",
    nextBlockHash:
        "00000000dd9588e8b12025e5257267abe0e9134d493ab3b449a47db8d22567d2",
    height: 579,
    confirmations: 611337,
    size: 216,
    time: 1232023318,
    version: 1,
    merkleRoot:
        "da51ca935176551082fe7f51f5d05fe8019df19f108de387767ae509293018cf",
    nonce: "3302859286",
    bits: "1d00ffff",
    difficulty: "1",
    txCount: 1,
    txs: [
        {
            txid:
                "da51ca935176551082fe7f51f5d05fe8019df19f108de387767ae509293018cf",
            vin: [{ n: 0, isAddress: false, value: "0" }],
            vout: [
                {
                    value: "5000000000",
                    n: 0,
                    addresses: ["1GGWbaXJskxYasdVTvLJsUDD6s99uBDkGr"],
                    isAddress: true
                }
            ],
            blockHash:
                "0000000074e77a1e561ab5247bec8a977977d3dde3d70a74fabf0adc3d6bf4ac",
            blockHeight: 579,
            confirmations: 611337,
            blockTime: 1232023318,
            value: "5000000000",
            valueIn: "0",
            fees: "0"
        }
    ]
};
