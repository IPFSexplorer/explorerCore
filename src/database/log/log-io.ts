...
export default class LogIO {
    ...
    static async toMultihash(log: Log, { format = undefined } = {}) {
        ...
        if ((await log.getValues()).length < 1)
    ...
    static async fromEntry(
        ...
        length =
            length > -1
                ? Math.max(length, sourceEntries.length)
                : length;
        ...
        const uniques = findUniques(combined, "hash").sort(
            Entry.compare,
        );

        // Cap the result at the right size by taking the last n entries
        const sliced = uniques.slice(
            length > -1 ? -length : -uniques.length,
        );

        // Make sure that the given input entries are present in the result
        // in order to not lose references
        const missingSourceEntries = difference(
            sliced,
            sourceEntries,
            "hash",
        );
